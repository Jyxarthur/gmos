...
class PositionEmbeddingRandom(nn.Module):
    ...
    def forward(self, size: Tuple[int, int]) -> torch.Tensor:
        ...
        # Deterministic replacement for cumsum on a grid of ones
        y_embed = torch.arange(1, h + 1, device=device, dtype=torch.float32).view(h, 1).expand(h, w) - 0.5
        x_embed = torch.arange(1, w + 1, device=device, dtype=torch.float32).view(1, w).expand(h, w) - 0.5
        y_embed = y_embed / h
        x_embed = x_embed / w
